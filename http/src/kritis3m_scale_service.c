...
int parse_configuration(char *response, int response_len)
{
  ...
  if (cJSON_IsObject(system_config))
  {
    ...
    // Final Checks 
    // certificat matches 
    outer: for (int i = 0; i< system_configuration.number_proxy_applications; i++){

    ret = -1;
       inner: for (int j=0 ; j< system_configuration.number_crypto_profiles; j++){
  ...
  int parse_proxy_appl(cJSON * js_proxy_appl, ProxyApplication * sys_proxy_appl)
  {
    ...
    if (cJSON_IsArray(connection_whitelist))
    {
      ...
      for (int i = 0; i < number_connections, i++)
      {
        ...
        {
          strcpy(sys_proxy_appl->connection_whitelist.allowed_connections[i].allowed_client_ip_port, js_item->valuestring);
        }
        else
        {
