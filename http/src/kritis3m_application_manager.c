...
int start_standard_application(struct SystemConfiguration *configuration)
{
    ...
    for (int i = 0; i < configuration->standard_applications; i++)
    {
        ...
        case ECHO_TCP_SERVER:
            ...
            ret = parse_IPv4_fromIpPort(configuration->standard_applications[i].listening_ip_port, config.own_ip_address);
            if (ret < 0)
            {
                LOG_ERROR("Failed to parse IP address from listening IP port");
                return -1;
...
int start_proxy_applications(struct SystemConfiguration *configuration)
{
    ...
    for (int i = 0 ; i < configuration->number_proxy_applications; i++)
    {
        ...
        proxy_config.application_type = configuration->proxy_applications[i].application_type;

        int ret = parse_IPv4_fromIpPort(configuration->proxy_applications[i].listening_ip_port, proxy_config.own_ip_address);
        if (ret < 0)
        {
            LOG_ERROR("Failed to parse IP address from listening IP port");
            return -1;
        ...
        ret = parse_IPv4_fromIpPort(configuration->proxy_applications[i].target_ip_port, proxy_config.target_ip_address);
        if (ret < 0)
        {
        ...
        ret = parse_asl_from_crypto_profile(configuration->proxy_applications[i].tunnel_crypto_profile_ID, &tls_config);

        proxy_config

        // start proxy
        ret = proxy_run(&proxy_config);
        if (ret < 0)
        {
