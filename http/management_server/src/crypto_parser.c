...
int read_certificates(crypto_identity* identity)
{
        ...
        ret = create_file_path(filepath,
                               MAX_FILEPATH_SIZE,
                               identity->filepath,
                               identity->filepath_size,
                               "cert.pem",
                               sizeof("cert.pem"));
        if ((ret >= 0) && read_file(filepath,
                                    &identity->certificates.root_buffer,
                                    (int*) &identity->certificates.root_buffer_size) != 0)
        {
                return -1;
        }
        ...
        if (read_file(filepath,
                      &identity->certificates.key_buffer,
                      (int*) &identity->certificates.key_buffer_size) != 0)
        {
        ...
        if (read_file(filepath,
                      &identity->certificates.chain_buffer,
                      (int*) &identity->certificates.chain_buffer_size) != 0)
        {
                free(identity->certificates.root_buffer);
        ...
        if (read_file(filepath,
                      &identity->certificates.additional_key_buffer,
                      (int*) &identity->certificates.additional_key_buffer_size) != 0)
        {
