...
proxy_config tls_proxy_default_config(void)
{
        ...
        default_config.incoming_ip_address = NULL;
        default_config.incoming_port = 0; /* 0 selects random available port */
        default_config.outgoing_ip_address = NULL;
        default_config.outgoing_port = 0;
        default_config.log_level = LOG_LVL_WARN;
        default_config.incoming_tls_config = asl_default_endpoint_config();
        default_config.outgoing_tls_config = asl_default_endpoint_config();
...
int tls_proxy_start(proxy_config const* config)
{
        ...
        request.type = PROXY_START_REQUEST;
        request.payload.proxy_config = *config;
