...
static proxy *find_proxy_by_application_id(int application_id)
{
        for (int i = 0; i < MAX_PROXYS; i++)
        {
                if (proxy_pool[i].application_id == application_id)
                {
                        return &proxy_pool[i];
                }
        }
...
static int handle_management_message(proxy_backend *backend, int socket, proxy_management_message const *msg)
{
        ...
        case PROXY_STATUS_APPL_ID_REQUEST:
        {
                /* Find the proxy */
                proxy *proxy = find_proxy_by_application_id(msg->payload.status_req.proxy_id);
        ...
        case PROXY_STOP_APPL_ID_REQUEST:
        {
                /* Kill the proxy */
                proxy *proxy_to_be_killed = find_proxy_by_application_id(msg->payload.proxy_id);
                if (proxy_to_be_killed != NULL)
                {
                        poll_set_remove_fd(&backend->poll_set, proxy_to_be_killed->incoming_sock);
                        kill_proxy(proxy_to_be_killed);
