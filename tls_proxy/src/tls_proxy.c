...
static int send_management_message(int socket, tls_proxy_management_message const* msg)
{
        ...
        if (ret < 0)
        {
                LOG_ERROR("Error sending message: %s", strerror(errno));
                return -1;
        }
...
static int read_management_message(int socket, tls_proxy_management_message* msg)
{
        ...
        if (ret < 0)
        {
                LOG_ERROR("Error receiving message: %s", strerror(errno));
                return -1;
        }
...
static int add_new_proxy(enum tls_proxy_direction direction, proxy_config const* config)
{
        ...
        if (ret != 0)
                ERROR_OUT_EX(proxy->log_module, "Error adding new proxy to poll_set");
                        ...
                        else if ((proxy_connection = find_proxy_connection_by_fd(fd)) != NULL)
                        {
                                if ((event & POLLIN) || (event & POLLOUT))
                                {
                                        ...
                                        if (ret == ASL_SUCCESS)
                                        {
                                                ...
                                                if (ret != 0)
                                                {
                                                        LOG_ERROR("Error starting client handler thread: %s", strerror(ret));
                                                        proxy_connection_cleanup(proxy_connection);
                                                }
                                        }
                                        ...
                                        while (ret > 0)
                                        {
                                                ...
                                                if (ret > 0)
                                                {
                                                        ...
                                                        if (ret == -1)
                                                        {
                                                                ...
                                                                {
                                                                        LOG_ERROR_EX(*connection->log_module, "Error sending data to asset: %s",
                                                                                   strerror(errno));
                                                                        shutdown = true;
                                                                        break;
                                                                }
...
int tls_proxy_backend_run(proxy_backend_config const* config)
{
        ...
        if (ret < 0)
        {
                LOG_ERROR("Error creating socket pair for management: %s", strerror(errno));
                return -1;
        }
        ...
        {
                LOG_ERROR("Error starting TLS proxy thread: %s", strerror(ret));
        }
