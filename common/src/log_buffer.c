...
static int (*send_log_cb)(const char* message) =NULL;
...
static void* flush_thread_func(void* arg)
{
        ...
        while (buffer->running)
        {
                ...
                if (batch_count > 0 && batch_pos < sizeof(batch_buffer) - 1)
                {
                        ...
                        if (send_log_cb && (send_log_cb(batch_buffer) != 0))
                        {
                                LOG_WARN("Failed to send log batch to MQTT broker");
                        }else{
                            printf("sent log batch to MQTT broker error: %p\n", send_log_cb);
                        }
                }
                else
                {
