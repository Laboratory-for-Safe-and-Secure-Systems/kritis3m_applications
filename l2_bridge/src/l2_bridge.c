...
Z_KERNEL_STACK_DEFINE_IN(bridge_stack,
                         STACK_SIZE,
        ...
        while (1)
        {
                ...
                for (int i = 0; i < bridge->poll_set.num_fds; i++)
                {
                        ...
                        if (fd == bridge->lan_socket)
                        {
                                if (event & POLLIN)
                                {
                                        recv_buffer_len = recvfrom(bridge->lan_socket,
                                                                   recv_buffer,
                                                                   sizeof(recv_buffer),
                                                                   0,
                                            ...
                                            (source.sll_pkttype == PACKET_BROADCAST))
                                        {
                                                ...
                                                if (ret < 0)
                                                {
                                                        LOG_ERROR("Failed to send to WAN "
                                                                  "interface, errno %d",
                                                                  errno);
                                                        break;
                                                }
                        ...
                        else if (fd == bridge->wan_socket)
                        {
                                if (event & POLLIN)
                                {
                                        recv_buffer_len = recvfrom(bridge->wan_socket,
                                                                   recv_buffer,
                                                                   sizeof(recv_buffer),
                                                                   0,
                                                                   (struct sockaddr*) &source,
                                            ...
                                            (source.sll_pkttype == PACKET_BROADCAST))
                                        {
                                                ...
                                                if (ret < 0)
                                                {
                                                        LOG_ERROR("Failed to send to LAN "
                                                                  "interface, errno %d",
                                                                  errno);
                                                        break;
                                                }
...
int l2_bridge_run(l2_bridge_config const* config)
{
        ...
        if (bind(theBridge.lan_socket,
                 (struct sockaddr*) &theBridge.lan_interface,
                 sizeof(theBridge.lan_interface)) < 0)
        {
        ...
        if (bind(theBridge.wan_socket,
                 (struct sockaddr*) &theBridge.wan_interface,
                 sizeof(theBridge.wan_interface)) < 0)
        {
...
#if !defined(__ZEPHYR__)
        if (setsockopt(theBridge.lan_socket, SOL_PACKET, PACKET_IGNORE_OUTGOING, &(int) {1}, sizeof(int)) <
            0)
        {
        ...
        if (setsockopt(theBridge.wan_socket, SOL_PACKET, PACKET_IGNORE_OUTGOING, &(int) {1}, sizeof(int)) <
            0)
        {
