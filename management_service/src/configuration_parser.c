                                   ...
                                   char* identity_path)
{
        ...
        if ((js_middleware_path == NULL) || (strcmp(js_middleware_path->valuestring, "") == 0))
        {
        ...
        if ((js_pin == NULL) || (strcmp(js_pin->valuestring, "") == 0))
        {
                LOG_DEBUG("no middleware path provided");
                config->pin = NULL;
                config->pin_size = 0;
        ...
        char* json_parsed = strncpy(config->serial_number,
                                    cJSON_GetObjectItem(json_management_service, "serial_number")->valuestring,
                                    SERIAL_NUMBER_SIZE);
        if (json_parsed == NULL)
        {
                goto error_occured;
        ...
        get_identity_folder_path(config->identity.filepath,
                                 config->identity.filepath_size,
                                 identity_path,
...
int parse_buffer_to_Config(char* json_buffer, int json_buffer_size, Kritis3mNodeConfiguration* config)
{
        ...
        if (json_management_config == NULL)
                goto error_occured;
        ret = parse_json_to_ManagementConfig(json_management_config,
                                             &config->management_identity,
                                             config->pki_cert_path);
        if (ret < 0)
                goto error_occured;
                                                         ...
                                                         char* pin)
{
        ...
        {
                LOG_WARN("can't take over all ip addr, since store is just designed for %d ip "
                         "addresses",
                         MAX_NUMBER_HW_CONFIG);
        ...
        for (int i = 0; i < number_hw_configs; i++)
        {
                ...
                strncpy(config->application_config.hw_config[i].device,
                        json_dev->valuestring,
                        IF_NAMESIZE);

                cJSON* json_ip_cidr = cJSON_GetObjectItem(hw_config_item, "cidr");
                if (json_ip_cidr == NULL)
                        goto error_occured;
                strncpy(config->application_config.hw_config[i].ip_cidr,
                        json_ip_cidr->valuestring,
                        INET6_ADDRSTRLEN + 4);
        }
        ...
        for (int i = 0; i < number_crypto_profiles; i++)
        {
                ...
                retval = parse_crypo_config(crypto,
                                            &config->application_config.crypto_profile[i],
                                            secure_middleware_path,
                                            pin);
                if (retval < MGMT_OK)
                        goto error_occured;
        ...
        for (int i = 0; i < number_crypto_identities; i++)
        {
                ...
                ManagementReturncode returncode = parse_crypo_identity(crypto_identity,
        ...
        {
                LOG_WARN("Too many appls provided. Can't init %d applications ",
                         MAX_NUMBER_APPLICATIONS - number_applications);
...
ManagementReturncode parse_whitelist(cJSON* json_obj, Whitelist* whitelist)
{
        ...
        int number_trusted_clients = cJSON_GetArraySize(
                cJSON_GetObjectItem(json_obj, "trusted_clients"));
        whitelist->number_trusted_clients = number_trusted_clients;
        for (int i = 0; i < number_trusted_clients; i++)
        {

                cJSON* trusted_client_json = cJSON_GetArrayItem(cJSON_GetObjectItem(json_obj, "trusted_clients"),
                                                                i);
                ...
                int retval = parse_addr_toKritis3maddr(client_ip->valuestring,
                                                       &whitelist->TrustedClients[i].trusted_client);
                if (retval < 0)
                {
                        goto error_occured;
                }

                int number_trusted_applications = cJSON_GetArraySize(
                        cJSON_GetObjectItem(trusted_client_json, "application_ids"));
                ...
                for (int j = 0; j < number_trusted_applications; j++)
                {
                        ...
                        if (trusted_applications == NULL)
                                goto error_occured;
                        whitelist->TrustedClients[i]
                                .trusted_applications_id[j] = trusted_applications->valueint;
                }
                                          ...
                                          char* crypto_identity_path)
{
        ...
        ret = get_identity_folder_path(identity->filepath,
                                       identity->filepath_size,
                                       crypto_identity_path,
                                       identity->identity);
        if (ret < 0)
                goto error_occured;
